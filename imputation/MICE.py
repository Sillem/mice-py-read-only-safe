...
class MICE:
    ...
    def __init__(self, data):
        ...
        for col, stats in missing_stats.items():
            logger.debug(
                f"  {col}: {stats['missing_count']} values ({stats['missing_percentage']:.2f}%) missing"
            )
    ...
    def impute(
        ...
        **kwargs : dict
        ...
        logger.debug(
            f"Parameters: n_imputations={n_imputations}, maxit={maxit}, "
        ...
        if predictor_matrix is None:
            ...
            predictor_matrix = validate_predictor_matrix(
                predictor_matrix, list(self.data.columns), self.data
            )
        ...
        numeric_cols_to_impute = [
            col for col in self.visit_sequence if col in numeric_cols
        ]
        ...
        for chain_idx in range(self.n_imputations):
            ...
            logger.info(
                f"Starting imputation chain {chain_idx + 1}/{self.n_imputations}"
            )
            ...
            logger.info(
                f"Completed imputation chain {chain_idx + 1} in {chain_duration:.2f} seconds"
            )
        ...
        logger.info(
            f"All {self.n_imputations} imputations completed in {total_duration:.2f} seconds"
        )
        logger.info(f"Average time per imputation chain: {avg_chain_time:.2f} seconds")
        logger.debug(
            f"Individual chain times: {[f'{t:.2f}s' for t in individual_times]}"
        )
        ...
        logger.debug(
            f"  - Predictor matrix provided: {self.predictor_matrix is not None}"
        )
    ...
    def _quickpred(
        ...
        """
        logger.info(
            f"Estimating predictor matrix with min_cor={min_cor}, min_puc={min_puc}, method='{method}'"
        )

        predictor_matrix = pd.DataFrame(
            0, index=self.data.columns, columns=self.data.columns
        )
        ...
        for target_col in self.data.columns:
            ...
            for predictor_col in self.data.columns:
                ...
                if puc >= min_puc:
                    ...
                    if (
                        target_col in cor_matrix.index
                        and predictor_col in cor_matrix.columns
                    ):
        ...
        if include:
            ...
            if unknown_includes:
                raise ValueError(
                    f"_quickpred include contains unknown columns: {unknown_includes}"
                )
        ...
        if exclude:
            ...
            if unknown_excludes:
                raise ValueError(
                    f"_quickpred exclude contains unknown columns: {unknown_excludes}"
                )
            predictor_matrix.loc[:, exclude] = 0

        # Ensure diagonal is zero
        np.fill_diagonal(predictor_matrix.values.copy(), 0)
    ...
    def fit(self, formula: str) -> None:
        ...
        logger.info(
            f"Analysis completed successfully. Fitted models to {len(self.model_results)} datasets"
        )
        logger.debug(
            f"Model results stored in self.model_results with {len(self.model_results)} entries"
        )
    ...
    def pool(self, summ: bool = False):
        ...
        if not hasattr(self, "formula"):
            logger.warning(
                "No formula found. This may indicate .fit() was not called properly."
            )
        ...
        params_array = np.array(
            params_list
        )  # Shape: (m, p) where p = number of parameters
        ...
        if m > 1:
            ...
            logger.warning(
                "Only one imputation available. Between-imputation variance set to zero."
            )
        ...
        logger.debug(
            f"Computed within-imputation variance diagonal: {np.diag(cov_within)}"
        )
        logger.debug(
            f"Computed between-imputation variance diagonal: {np.diag(cov_between)}"
        )
        ...
        if hasattr(self.model_results[0], "model") and hasattr(
            self.model_results[0].model, "endog_names"
        ):
    ...
    def _impute_once(self, chain_idx: int):
        ...
        for iter_idx in range(self.maxit):
            logger.debug(
                f"Starting iteration {iter_idx + 1}/{self.maxit} for chain {chain_idx}"
            )
    ...
    def _iterate(
        ...
        for col in self.visit_sequence:
            logger.debug(
                f"Processing column '{col}' (iteration {iter_idx + 1}, chain {chain_idx})"
            )
            ...
            imputed_values = imputer_func(
                y=y, id_obs=id_obs, id_mis=id_mis, x=predictors, **method_params
            )
            logger.debug(
                f"Successfully imputed {len(imputed_values)} values for column '{col}'"
            )
            ...
            if id_mis.sum() > 0 and col in self.chain_mean:
                ...
                if imputed_arr.size > 1:
                    ...
                    logger.debug(
                        f"Chain statistics for '{col}': mean={mean_val:.4f}, variance={var_val:.4f}"
                    )
                else:
                    self.chain_var[col][iter_idx, chain_idx] = np.nan
                    logger.debug(
                        f"Chain statistics for '{col}': mean={mean_val:.4f}, variance=N/A (single value)"
                    )

        iteration_time = time.time() - iteration_start_time
        logger.debug(
            f"Iteration {iter_idx + 1} completed in {iteration_time:.2f} seconds"
        )
        return updated_data
        ...
        if self.initial == InitialMethod.SAMPLE.value:
            for col in data.columns:
                if data[col].isna().any():
                    observed_values = data.loc[self.id_obs[col], col].values
                    data.loc[self.id_mis[col], col] = rng.choice(
                        observed_values, size=self.id_mis[col].sum()
                    )
    ...
    def _set_visit_sequence(self, visit_sequence):
        ...
        columns_with_missing = [
            col for col in self.data.columns if self.data[col].isna().any()
        ]

        # Validate using centralized validator
        validated_sequence, cols_without_missing = check_visit_sequence(
            visit_sequence, list(self.data.columns), columns_with_missing
        )
        ...
        if isinstance(visit_sequence, list):
            ...
            if visit_sequence == VisitSequence.RANDOM.value:
                ...
                nmis = np.array(
                    [self.id_mis[col].sum() for col in columns_with_missing]
                )
                ii = np.argsort(nmis)
